		...
		<div key={fieldName}>
			<h3 className="text-lg font-semibold text-gray-600">
				{isExpandable ? (
					...
					>
						<span>{fieldName}:</span>
						<FaCaretDown className={clsx("transition-transform", isExpanded ? "rotate-0" : "-rotate-90")} />
					</button>
				) : (
					<span>{fieldName}:</span>
				)}
			...
			<div className="text-gray-500">
				{fieldValue?.contentID > 0 && fieldValue?.fields ? (
					...
					<div className="border border-gray-200 rounded-sm bg-white">
						<div className={clsx("transition-all overflow-auto", isExpanded ? "max-h-full" : "max-h-32 ")}>
					...
					<div className="border border-gray-200 rounded-sm bg-white overflow-auto">
						<div className={clsx("p-2 transition-all overflow-auto", isExpanded ? "max-h-full" : "max-h-32")}>
					...
					<div className={clsx(isExpanded ? "" : "max-h-20 overflow-auto")}>{fieldValue}</div>
				) : isJSON ? (
